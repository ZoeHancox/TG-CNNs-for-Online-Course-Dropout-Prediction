...
# To make sparse matrices from the data
...
conv_D_out_long = max_action - (num_time_steps - 1) # assuming stride=1 and padding =0
stride_fine = (2,1,1)
#conv_D_out_fine = int(((max_action - (num_time_steps - 1)-1)/stride_fine) + 1)
conv_D_out_fine = int(((max_action - (num_time_steps - 1)-1)/2) + 1)
...
class Dense3DConv(nn.Module):
    def __init__(self):
        ...
        # Fine path
        self.conv3d_fine = nn.Conv3d(in_chans, out_chans, kernel_size=(num_time_steps,97,97), stride=stride_fine, bias=False) 
        self.batchnorm = nn.BatchNorm3d(out_chans)
        self.lstm_fine = nn.LSTM(input_size = out_chans, hidden_size= lstm_h, batch_first = True)
        self.flat = nn.Flatten()
        self.fc1_fine = nn.Linear(conv_D_out_fine * lstm_h, linear_size) #  conv_out * hidden_size
        self.relu = nn.ReLU()
        self.fc2_fine = nn.Linear(linear_size, 256) # input chans, number of labels
        
        # Long path
        self.conv3d_long = nn.Conv3d(in_chans, out_chans, kernel_size=(num_time_steps,97,97), stride=1, bias=False) 
        self.lstm_long = nn.LSTM(input_size = out_chans, hidden_size= lstm_h, batch_first = True)
        self.fc1_long = nn.Linear(conv_D_out_long * lstm_h, linear_size) #  conv_out * hidden_size
        self.fc2_long = nn.Linear(linear_size, 256) # input chans, number of labels

        self.sig = nn.Sigmoid()
        self.dropout = nn.Dropout(p=drop_val)

        # Concatenation
        self.join = nn.Linear(256+256, 1) # input = sum of both channels combined.
        # ^ final fully connected layer
    

    def forward(self, x):
        x = x.to(device)
        gamma = ((gamma_max-gamma_min)*self.sig(self.gamma) + gamma_min) # range gamma within min and max
        scaled_matrix = torch.where(x==0, x, torch.exp(-gamma*x)) # condition for only non-zero elements
        
        # FINE CHANNEL
        out_fine = self.conv3d_fine(scaled_matrix)
        out_fine = self.batchnorm(out_fine)
        
        out_fine = torch.squeeze(out_fine, 4)
        out_fine = torch.squeeze(out_fine, 3)
        
        out_fine = self.relu(out_fine)        
        out_fine, cell_state = self.lstm_fine(torch.transpose(out_fine, 1, 2)) # switching the layout to suit LSTM switching the position of the sequence_length and out_chans
        
        out_fine = self.flat(out_fine)
        out_fine = self.dropout(out_fine)
        out_fine = self.fc1_fine(out_fine)
        out_fine = self.relu(out_fine)
        out_fine = self.dropout(out_fine)
        out_fine = self.fc2_fine(out_fine)

        # LONG CHANNEL
        out_long = self.conv3d_long(scaled_matrix)
        out_long = self.batchnorm(out_long)
        
        out_long = torch.squeeze(out_long, 4)
        out_long = torch.squeeze(out_long, 3)
        
        out_long = self.relu(out_long)        
        out_long, cell_state = self.lstm_long(torch.transpose(out_long, 1, 2)) # switching the layout to suit LSTM switching the position of the sequence_length and out_chans
        
        out_long = self.flat(out_long)
        out_long = self.dropout(out_long)
        out_long = self.fc1_long(out_long)
        out_long = self.relu(out_long)
        out_long = self.dropout(out_long)
        out_long = self.fc2_long(out_long)


        # CONCATENATION OF THE TWO CHANNELS
        out = torch.cat((out_fine, out_long), dim=1)
        out = self.relu(out)
        out = self.join(out)
...
avg_val_batch_prec = []
avg_train_batch_prec = []
avg_test_batch_prec = []

avg_val_batch_recall = []
avg_train_batch_recall = []
avg_test_batch_recall = []
...
for epoch in range(nepochs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        gamma_no = list(model.named_parameters())[0] # prints the self.gamma value (before put through sigmoid and min max of gamma)
        gamma_no_alone = torch.tensor(gamma_no[1].item())
        gamma = (gamma_max-gamma_min)*sig(gamma_no_alone) + gamma_min
        ...
        if debug:
            ...
            print(f"Current gamma value: {gamma:.5f}")
